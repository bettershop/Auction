  ...
  watch: {
    ...
    '$route.query': {
      ...
      handler(newQuery) {
        ...
        if (targetCateId || targetSubCateId || hasSearchParam ) {
          this.pageNo = 1;
          this.getProducts();
        }
      }
  ...
  //跳转到商品详情页
    goToProductDetail(product) {
      this.$router.push({
        ...
        query: {
          ...
          categoryId: this.selectedCategory?.cid || '', // 一级分类cid
          subCategoryId: this.selectedSubCategory?.cid || '', // 二级分类cid
        }
      });
    },
    // 设置当前导航标题
    setNavTitle() {
      ...
      const routeMap = {
        ...
        '/shop-session': this.$t("mchSession"),
    ...
    statusFilter(sessionStatus) {
      let value = '';
      if (sessionStatus === 1) {
        value = this.$t('auction.rsc');
      } else{
        value = this.$t('auction.cjj');
      }
      return value;
    },
    ...
    dateFilter(sessionStatus) {
      let value = '';
      if (sessionStatus === 1) {
        value = this.$t('auction.kpsj');
      } else{
        value = this.$t('auction.jpsj');
      }
      return value;
    },
    ...
    // 更新面包屑
    updateBreadcrumbs() {
      const currentPagePath = '/auction/single-auction'; // ✅ 强制写死当前页面路径
      this.breadcrumbs = [
        { name:this.$t('auction.sy'), type: 'home', path: '/' },
        { name: this.$t('oneAuction'), type: 'nav', path: currentPagePath }
      ];
      if (this.selectedCategory) {
        this.breadcrumbs.push({
          ...
          path: currentPagePath // ✅ 强制指向当前页面，永不指向allClass
        });
    ...
    //点击面包屑导航
    // 你的 single-auction.vue 里的方法，找到并删除 ↓↓↓ 这行代码！！！
